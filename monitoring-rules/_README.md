...
| `service` | the alert's SUBJECT (see "service / namespace labels") | OpenObserve incident correlation |

## service / namespace labels (2026-09-25)

Every alert names its **subject** in `service` (lint: `[RULES-SERVICE]`). OpenObserve
Incidents correlate alerts on `service` + `namespace` (measured: same service with a
missing namespace joins; same service with a DIFFERENT namespace splits; same namespace
alone never joins). Left alone, an alert inherits the scrape target's labels —
`service=pushgateway, namespace=monitoring` for every Windmill probe,
`rancher-monitoring-kube-state-metrics`, `rook-ceph-mgr` … — and O2 merged unrelated
alerts into one "pushgateway" incident. Choose the value, in this order:

1. **A Kubernetes workload** → the workload's own name, which is also O2's
   `service_name` for its pod logs. Template it from the series:
   `service: '{{ or $labels.deployment $labels.statefulset $labels.daemonset $labels.cronjob "<default>" }}'`.
   Do NOT add `$labels.container` or `$labels.pod`: pushgateway and kube-state-metrics
   series carry the EXPORTER's container name, which is the problem being fixed.
   For a rule over pushgateway metrics use the plain default string instead.
2. **Per-series subjects** → the series' own label: `node`, `instance` (probes),
   `persistentvolumeclaim`, `cronjob`, `job` (Windmill batch job).
3. **Otherwise a stable subject name, shared with the other alert sources** so they
   correlate: `bpi-r4` (Home Assistant's router alarms use it), `ceph` + `rook-ceph`,
   `home-assistant`, `batteries`, `fusionio`, `findmy-collector` (Windmill
   `f/lib/mdapi._O2_ALARM_SENDERS`), `gitlab-gitlab-runner` + `bootstrap`
   (ci-templates `notify.yml`).

`namespace` is set ONLY to replace the pushgateway artifact, and keeps any real value:
`namespace: '{{ if or (eq $labels.namespace "") (eq $labels.namespace "monitoring") }}<subject ns>{{ else }}{{ $labels.namespace }}{{ end }}'`.
Never set a static `namespace` on a rule whose series carry a real one — the
`OpenBaoMirrorStale` → `CronJobStale` inhibit joins on `[namespace, cronjob]`.

⚠️ `vmalert -dryRun` does NOT validate label templates (poison-tested). Prove a new
template by evaluating it: a scratch VMRule with `team: zz-template-test` (no route
matches) and `vector(1)` inputs, then read `/api/v1/alerts` + rule `lastError`.
