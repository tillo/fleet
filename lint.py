    ...
    for doc in load_docs(path):
        ...
        for grp in (doc.get("spec", {}).get("groups") or []):
            for rule in (grp.get("rules") or []):
                ...
                # `service` names the SUBJECT of the alert (2026-09-25). Without it an
                # alert inherits the scrape target's `service` — pushgateway,
                # kube-state-metrics, rook-ceph-mgr — and OpenObserve Incidents, which
                # correlate on service + namespace, merge unrelated alerts into one
                # "pushgateway" incident. Convention: monitoring-rules/_README.md.
                if not lab.get("service"):
                    errors.append(
                        f"[RULES-SERVICE] {where}: no `service` label — name the "
                        "affected workload/subject (see monitoring-rules/_README.md, "
                        "'service / namespace labels'), or the alert is attributed "
                        "to whatever exporter scraped the metric."
                    )
...
# ---------------------------------------------------------------------------
# (F) Every bundle directory must appear in fleet-paths.txt
#
# gitrepo/mdapi-prod enumerates spec.paths explicitly (a null paths renders an
# extra catch-all bundle that duplicates the whole repo against etcd's 1.5 MiB
# per-object ceiling). The permanent cost is that Fleet discovers NOTHING.
#
# On 2026-09-22 the falco/ bundle merged with a green pipeline and deployed
# nothing: no Bundle was ever created, while the GitRepo reported Ready=True and
# Stalled=False. Every signal said healthy. This check is the thing that would
# have caught it, in the MR, instead of an hour later.
# ---------------------------------------------------------------------------
PATHS_FILE = REPO / "fleet-paths.txt"
if PATHS_FILE.exists():
    # ⛔ Do NOT shell out to `git ls-files` here: the fleet-lint image has no git
    # binary, and this check died with FileNotFoundError the first time it ran in
    # CI while passing locally. rglob + the dot-part exclusion is the same filter
    # all_yaml_files() uses, and it drops .git and .claude/worktrees/* alike —
    # which matters, because a nested agent worktree is a full second copy of
    # every bundle.
    bundle_dirs = {
        str(p.parent.relative_to(REPO)).replace("\\", "/")
        for p in REPO.rglob("fleet.y*ml")
        if p.name in ("fleet.yaml", "fleet.yml")
        and not any(part.startswith(".") for part in p.relative_to(REPO).parts)
    }
    bundle_dirs.discard(".")          # a root fleet.yaml must NEVER be a path

    # Fleet RECURSES into every listed path, so a directory whose ancestor is
    # also a bundle must not be listed separately: listing both renders that
    # bundle twice in one git job, and the losing write stalls the GitRepo —
    # which freezes bundling for the entire repo, not just that bundle.
    def _covered(d: str) -> bool:
        parts = d.split("/")
        return any("/".join(parts[:i]) in bundle_dirs for i in range(1, len(parts)))

    expected = {d for d in bundle_dirs if not _covered(d)}
    listed = {
        ln.strip() for ln in PATHS_FILE.read_text().splitlines()
        if ln.strip() and not ln.lstrip().startswith("#")
    }

    for d in sorted(expected - listed):
        errors.append(
            f"[FLEET-PATHS] bundle directory '{d}/' is not in fleet-paths.txt, so "
            f"gitrepo/mdapi-prod will NOT render it — the MR would merge green and "
            f"deploy nothing. Add '{d}' to fleet-paths.txt, and after merge: "
            f"kubectl --context mdapi-rancher -n fleet-default patch "
            f"gitrepos.fleet.cattle.io mdapi-prod --type=json "
            f"-p '[{{\"op\":\"add\",\"path\":\"/spec/paths/-\",\"value\":\"{d}\"}}]'"
        )
    for d in sorted(listed - expected):
        why = ("it is covered by a listed ancestor (Fleet recurses)"
               if _covered(d) else "no fleet.ya?ml exists there any more")
        warnings.append(
            f"[FLEET-PATHS] fleet-paths.txt lists '{d}' but {why} — remove it here "
            f"and from the live GitRepo's spec.paths."
        )


# ---------------------------------------------------------------------------
# (G) No Keycloak BUILD-TIME option in the env of an OPTIMIZED Keycloak
#
# On 2026-09-22 (fleet !438) and again 2026-09-23 14:12Z, KC_TRACING_ENABLED /
# KC_TRACING_SAMPLER_TYPE were added to keycloak/keycloak-deploy.yml. Both are
# build-time options. The image runs `start --optimized` (options baked in by
# `kc.sh build` in the mdapi/keycloak Dockerfile), and an optimized server that
# is handed a build-time option whose value differs from the baked one prints
# ONE line and exits — both replicas CrashLoopBackOff, SSO down estate-wide.
# Build-time options belong in the Dockerfile ENV block before `kc.sh build`.
#
# ⭐ The list is DERIVED, not recalled: it is the option set printed by
# `kc.sh build --help-all` of the image in use (Keycloak 26.7.4, captured
# 2026-09-23 from the running keycloak pod). `build --help-all` lists exactly
# the build-time options. Re-capture it on a Keycloak MAJOR/MINOR bump:
#   kubectl -n keycloak exec deploy/keycloak -- /opt/keycloak/bin/kc.sh build --help-all
# Env form: KC_ + upper-cased option with '-' -> '_'. `<name>`/`<datasource>`
# options are prefix families.
#
# Scope: only containers started with `--optimized`. envuassu's keycloak runs
# plain `start` (auto-build at boot), where build-time env is legitimate.
# ---------------------------------------------------------------------------
KC_BUILD_EXACT = {
    "KC_DB", "KC_DB_DRIVER", "KC_EVENT_METRICS_USER_ENABLED", "KC_FEATURES",
    "KC_FEATURES_DISABLED", "KC_FIPS_MODE", "KC_HEALTH_ENABLED",
    "KC_HTTP_MANAGEMENT_HEALTH_ENABLED", "KC_HTTP_MANAGEMENT_RELATIVE_PATH",
    "KC_HTTP_RELATIVE_PATH", "KC_HTTPS_CLIENT_AUTH", "KC_HTTPS_MANAGEMENT_CLIENT_AUTH",
    "KC_LEGACY_OBSERVABILITY_INTERFACE", "KC_LOG_MDC_ENABLED", "KC_METRICS_ENABLED",
    "KC_OPENAPI_ENABLED", "KC_OPENAPI_UI_ENABLED", "KC_TELEMETRY_LOGS_ENABLED",
    "KC_TELEMETRY_METRICS_ENABLED", "KC_TRACING_ENABLED", "KC_TRACING_JDBC_ENABLED",
    "KC_TRACING_SAMPLER_TYPE", "KC_TRANSACTION_XA_ENABLED", "KC_VAULT",
}
KC_BUILD_PREFIX = (
    "KC_DB_DRIVER_", "KC_DB_HEALTH_EXCLUDE_", "KC_DB_KIND_", "KC_FEATURE_",
    "KC_TRANSACTION_XA_ENABLED_",
)
# Grandfathered: a build-time option whose runtime value is IDENTICAL to the
# baked one is accepted by Keycloak (the check is "differs from what is
# persisted"). Only KC_DB is allowed, and only at this exact value — it has
# been `postgres` in the Dockerfile since the image's first build. Anything
# that moves with the image (tracing, features, health, metrics) is NOT
# allowlisted: keycloak is Lane-A (:latest via Keel), so the baked value can
# change under a manifest that did not, which is the 09-22 failure shape.
KC_BUILD_ALLOW = {"KC_DB": "postgres"}


def kc_is_build_time(name: str) -> bool:
    return name in KC_BUILD_EXACT or name.startswith(KC_BUILD_PREFIX)
...
for path in all_yaml_files():
    for doc in load_docs(path):
        ...
        for c in spec.get("containers", []) or []:
            if not isinstance(c, dict):
                continue
            argv = [str(a) for a in (c.get("command") or []) + (c.get("args") or [])]
            if "keycloak" not in str(c.get("image", "")) or "--optimized" not in argv:
                continue
            for ev in c.get("env", []) or []:
                if not isinstance(ev, dict):
                    continue
                en = str(ev.get("name", ""))
                if not kc_is_build_time(en):
                    continue
                if en in KC_BUILD_ALLOW and "valueFrom" not in ev \
                        and str(ev.get("value")) == KC_BUILD_ALLOW[en]:
                    continue
                errors.append(
                    f"[KC-BUILD-TIME] {path.relative_to(REPO)}: {name}/"
                    f"{c.get('name', '?')} sets {en}, a Keycloak BUILD-TIME option, "
                    f"on a container started with --optimized. An optimized "
                    f"Keycloak EXITS on a build-time value that differs from the "
                    f"baked one (SSO outage 2026-09-22). Put it in the "
                    f"mdapi/keycloak Dockerfile ENV block before `kc.sh build`."
                )
