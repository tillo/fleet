...
def _akeyless(name):
    ...
    Until 2026-09-02 this shelled out to `ssh mbptillo akeyless ...` because
    the machine running this script did not carry Akeyless credentials.
    mbptillo has been permanently retired, and the seat that replaced it —
    jump — carries its own working Akeyless profile (fixed 2026-08-08: the
    CLI ignores the profile's gateway_url key, so AKEYLESS_GATEWAY_URL has to
    be in the environment, and it was below .bashrc's non-interactive guard).
    That is why the gateway is passed explicitly here rather than trusted to
    the caller's shell: this script is run by hand, sometimes non-interactive,
    and a missing gateway fails as a confusing 404 on /api/derived-key rather
    ...
    try:
        ...
        raise SystemExit(
            ...
            "(~/.local/bin/akeyless), which holds the credentials — the old "
            "'ssh mbptillo' proxy died with that machine on 2026-09-02.")
