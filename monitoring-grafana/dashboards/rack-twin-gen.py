...
## SALT (.207) and PEPPER (.219) were dropped here on 2026-09-21: both hosts are
## retired and their rack rows below already render as "retired" with no targets,
## so the constants had no reader left. Kept out rather than commented out — an
## address constant is the kind of thing that gets copied into a new query by
## someone who assumes it still points somewhere.
SW, MSM = "192.168.1.250", "192.168.1.15"
...
## ⛔ `scpu()` and `smem()` were DELETED here on 2026-09-21, with the last of
## their callers. They built UCD-SNMP queries (`ssCpuIdle`, `memAvailReal`), and
## salt and pepper were the only two hosts on this board that had no
## node-exporter; the `host_snmp` scrape went with them. Removed rather than left
## behind for the same reason their IP constants were, a few lines up: a helper
## that still compiles is the kind of thing someone reaches for when adding a
## row, and this one would have returned a query no exporter answers any more.
##
## ⭐ The lesson they carried is NOT lost with them — it is in memory
## [[ucd_snmp_memory_is_memfree_not_memavailable_2026_09_06]]: UCD's
## `memAvailReal` is MemFree and excludes buffers/cache, while node-exporter's
## `MemAvailable` includes reclaimable cache, so the two read systematically
## differently under one threshold band. That trap applies to the NEXT SNMP host
## put on this board, which is exactly why it belongs in memory and not in a
## comment attached to dead code.
def link(a): return f'ifOperStatus{{instance="{SW}",ifAlias="{a}"}}'
def hass(m, e): return f'{m}{{entity="{e}"}}'
## Watts per chassis, from f/probe/chassis_health: sensor "Avg Power" out of the
## SDR it already fetches over IPMI. It covered five hosts until 2026-09-20 —
## the x3550s had no iLO and no node-exporter, and their IMM answered IPMI, so
## this was the ONLY uniform power source across both hardware generations.
## Since their retirement it reads the three DL360s and nothing else.
...
## could never add up to. One source for every chassis on the board.
...
RACK = [
    ...
    ## ⛔ RETIRED 2026-09-20 (pepper 09-18, salt 09-20). Both boxes are powered
    ## off but STILL RACKED — Part C of ~/salt-shutdown-runbook.md is "abandon
    ## in place", not "erase": their disks still hold the pre-2026-09-19 Longhorn
    ## backup generations, and the recorded decision is that the box can be
    ## booted again if historical data is ever needed. So they keep their U rows
    ## and go "retired", exactly like bomberos/fntillo/vortex.
    ##
    ## Their cpu/mem came from the `host_snmp` scrape, which was deleted with
    ## them, and their W from the chassis probe, which no longer lists them.
    ## Leaving the panels would have rendered no-data on the estate's status
    ## board — and a no-data panel reads as "broken monitoring", not as
    ## "deliberately gone".
    (14, 14, "retired", "SALT   x3550 M2", "", []),
    (12, 12, "retired", "PEPPER   x3550 M2", "", []),
...
STYLE = {
    ...
    ## green started at 30; orange 10–30, red below 10.
    ## 2026-09-08: green 30 -> 20. The controller's cap moved 26->30 to 26->32,
    ## which drops typical output to 25–50 % and makes 20 % (its floor) an
    ## ordinary quiet night rather than a fault. Green must start at the floor.
    "fan":   [th([st(None, "red"), st(10, "orange"), st(20, "green")]), unit("percent"), dec(0)],
    "link":  [MAP_LINK, FLAT],
    ...
    ## How many metered chassis are MISSING from the sum. 0 is healthy; anything
    ## above 0 silently inflates the residual by that host's draw.
    ##
    ## ⛔⛔ This row used to be `count(chassis_power_watts)` with green hardcoded
    ## at 5, and salt+pepper's retirement (2026-09-20) turned it into a permanent
    ## red "3 /5 metered" on the estate's status board. That is the worse half of
    ## the failure: the red was not just wrong, it was OCCUPYING the only signal
    ## that says "one chassis stopped reporting", so a real IPMI failure would
    ## have changed nothing on screen. Retiring hardware disarmed the check that
    ## watches it — see [[monitoring_asserts_the_policy_you_are_changing_deploy_check_first_2026_09_11]].
    ##
    ## ⭐ So the expected count is no longer written down here. f/probe/chassis_health
    ## emits chassis_unreachable{host} for EVERY host in its HOSTS list — including
    ## ones it failed to reach — and chassis_power_watts only for those that
    ## answered with an "Avg Power" sensor. The difference IS the deficit, and it
    ## tracks the probe's own host list: retire a host and the expected count
    ## falls on its own; add one and the row goes red until it actually reports.
    ##
    ## ⛔ `or vector(0)` is not decoration. count() of an absent selector returns
    ## EMPTY, not 0, so if ALL chassis stopped reporting watts the subtraction
    ## would yield nothing and the panel would go BLANK — which reads as "no data
    ## yet", not as a fault. Poison-tested 2026-09-21 against VictoriaMetrics:
    ## healthy 0 · one host's watts gone 1 · all watts gone 3 · and without the
    ## `or vector(0)`, EMPTY. Same trap, same shape, as the smem() note that used
    ## to live at the top of this file.
    "hosts": [th([st(None, "green"), st(1, "red")]), unit("suffix: not metered"), dec(0)],
    ...
    ## the deficit above can NEVER notice a dead probe: both counts are retained
    ## together, so it stays 0 and green forever. timestamp() is no help either —
    ## it returns the pushgateway's own scrape time, so every series always looks
    ## ~3 s fresh. The real signal is push_time_seconds{job="chassis_health"}
    ## (exactly one series; 73 s old when measured 2026-09-05). The probe pushes
    ## every 15 min, so: yellow at one missed cycle (1200 s), orange at 2400, red
    ## at an hour. The DEFICIT above catches "probe runs but one host fails
    ## IPMI"; THIS row catches "probe stopped".
...
##   metered             every IPMI chassis + the PoE the XikeStor delivers
##   chassis reporting   guard: how many chassis are MISSING from the metered sum
...
## missing — that is what "chassis reporting" is for. If it reads 1 the residual
## is overstated by a whole server and the model error will scream.
## ⛔ Neither this block nor the style beside it writes down HOW MANY chassis
## there should be. It was five until 2026-09-20 and is three now; the count is
## derived from the probe's own host list so that the next retirement does not
## leave a red row behind. See the "hosts" style for the whole story.
##
...
rrow("pwr_hosts", "rack:power_metered_hosts:missing", "chassis reporting", "hosts")
...
## the deficit above cannot answer that.
## ⛔ `max by (job)` is not decoration. A pushgateway grouping key whose push was
## REJECTED is registered with push_time_seconds=0, so a bare selector would make
## this cell read ~1.79e9 s (≈56 years) and paint it red, permanently — the
## phantom survives until the group is deleted by hand. Live example found
## 2026-09-21: {job="lm_doctor", host="mbp-mda"}, a grouping key that had NEVER
## had a successful push. max() takes the most recent push and ignores it, and
## also collapses a genuinely multi-instance group to one number.
rrow("pwr_age", 'time() - max by (job) (push_time_seconds{job="chassis_health"})',
     "chassis feed age", "age")
