...
Found three times now, each time by accident, each time fixed one manager at a time:
...
KNOWN_UNREACHABLE = {
    ...
    ("openbao/fleet.yaml", "zot.mdapi.ch/mdapi/openbao"): "pinned twice in one fleet.yaml; bumping one shape alone would skew server vs unsealer",
}
